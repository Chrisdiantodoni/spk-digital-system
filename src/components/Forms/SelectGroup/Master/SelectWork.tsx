...
export default function SelectWork({ value, onChange, disabled }: Props) {
  const [search, setSearch] = useState('');
  const [optionsWork, setOptionsWork] = useState<any>([]);
  ...
  const { mutate: getWork, isPending } = useMutation({
    mutationKey: ['select_work'],
    mutationFn: async () => {
      const response = await master.getOccupation(resultQueryString);
