...
export default function SelectUnitNew({
  value,
  onChange,
  motor_id,
  without_motor_id = false,
}: Props & { without_motor_id?: boolean }) {
  ...
  useEffect(() => {
    if (motor_id) {
      getUnit();
    } else if (without_motor_id) {
      getUnit();
    } else {
      setOptionsUnit([]);
    }
  }, [debouncedSearchValue, motor_id, without_motor_id]);
  ...
  return (
    <div className=" w-full">
      <Select
        ...
        onChange={onChange}
        value={motor_id ? value : without_motor_id ? value : null}
        options={optionsUnit}
